@extends('layouts.app')
@if(isset($menu))
    @section('page-title')| {!! isset($menu)?$menu->name:"" !!}@endsection
@else
            ...
            <section class="content">
                <div class="row">
                    <div class="col-12 col-sm-3 sidebar_div_main" style="padding-right: 0;background-color: #F5F5F5">
                        @include('shop-menu.menu-sidebar')
                    </div>
                    <div class="col-12 col-sm-9 p-15">
                        <form id="save-outlet" method="POST" action="{!! env('APP_URL') !!}save/outlet" enctype="multipart/form-data">
                            ...
                            @php
                              ...
                              // $image = null;
                               //    if(isset($menu)){
                               //       $image = \App\Photos::where('resto_id',\App\Helpers\CommonMethods::getRestuarantID())->where('branch_id',$menu->id)->first();
                                ...
                                <div class="row mt-4">
                                    <div class="col-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" value="{!! isset($menu)?$menu->title:"" !!}" class="form-control" name="title"
                                                   required/>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="checkbox" id="md_checkbox_21" class="filled-in chk-col-primary" name="status" checked />
                                            <label for="md_checkbox_21">{{__('label.status')}}</label>
                                        </div>

                                        <div class="form-group">
                                            <label>{{__('label.phone')}}</label> <br />
                                            <input type="text" class="form-control" name="phone" value="{!! isset($menu)?$menu->phone_number:"" !!}" required />
                                        </div>

                                        <div class="form-group">
                                            <label>{{__('label.whatsapp')}}</label> <br />
                                            <input type="text" class="form-control" name="whatsapp_number" value="{!! isset($menu)?$menu->whatsapp_number:"" !!}" required />
                                        </div>
                                            ...
                                            <select class="form-select" required name="country_id">
                                                ...
                                                @if(isset($countries) && $countries->count() > 0)
                                                    @foreach($countries as $country)
                                                        <option value="{!! $country->id !!}" @if(isset($menu) && $menu->country_id==$country->id) selected @endif>{!! $country->country_name !!}</option>
                                                    @endforeach
                                                @endif
                                            ...
                                            <select name="time_zone" class="form-control" required>
                                                ...
                                                @if(isset($time_zones) && $time_zones->count() > 0)
                                                    @foreach($time_zones as $timezone)
                                                        <option value="{!! $timezone->timezone !!}" @if(isset($menu) && $menu->time_zone==$timezone->timezone) selected @endif>{!! $timezone->timezone !!}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                        ...
                                        @php
                                            ...
                                            $resto_metas = \App\RestoMetaDefs::where('parent_meta_def_id',0)->whereIn('meta_def_name',['BUSSINESS_CCY','DEFAUT_LANGUAGE'])->get();


                                            $existing_resto_meta = [];
                                            if(isset($menu))
                                            $existing_resto_meta = \App\RestoMetas::where('bussiness_id',$restaurant->id)->where('outlet_id',$menu->id)->pluck('meta_def_id')->toArray();
                                           ...
                                           // dump($existing_resto_meta);
                                            ...
                                            $existing_resto_meta_value = null;
                                             if(isset($menu))
                                            $existing_resto_meta_value = \App\RestoMetas::where('bussiness_id',$restaurant->id)->where('outlet_id',$menu->id)->get();
                                            $v = [];
